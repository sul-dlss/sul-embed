  ...
  describe 'message' do
    context 'with location restricted' do
      ...
      it 'has a message of location restricted' do
        expect(file_viewer.message[:message]).to eq 'Access is restricted to the Special collections reading room. See Access conditions for more information.'
        expect(file_viewer.message[:type]).to eq 'location-restricted'
      end
    end

    context 'with location restricted and stanford only' do
      let(:purl) do
        build(:purl, :location_restriction, :stanford_only_unrestricted, contents: [build(:resource, :file, files: [build(:resource_file)])])
      end

      it 'has a message for stanford users' do
        expect(file_viewer.message[:message]).to eq 'Access is restricted to Stanford-affiliated patrons.'
        expect(file_viewer.message[:type]).to eq 'stanford'
      end
    end
    ...
    context 'with location restricted on a file' do
      ...
      it 'has no message' do
        expect(file_viewer.message).to be false
      end
    end
  end
