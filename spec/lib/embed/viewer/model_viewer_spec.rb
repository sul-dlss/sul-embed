  ...
  let(:contents) do
    [
      instance_double(Embed::Purl::Resource, three_dimensional?: true,
                                             files: [instance_double(Embed::Purl::ResourceFile, title: 'obj1.glb', file_url: '//file/druid:abc123/obj1.glb', not_no_download?: true)])
    ]
  end
  ...
  describe '#three_dimensional_files' do
    ...
    context 'with no download' do
      let(:contents) do
        [
          instance_double(Embed::Purl::Resource, three_dimensional?: true,
                                                 files: [instance_double(Embed::Purl::ResourceFile, title: 'obj1.glb', file_url: '//file/druid:abc123/obj1.glb', not_no_download?: false)])
        ]
      end
  ...
  describe 'message' do
    context 'with location restricted' do
      ...
      it 'has a message of location restricted' do
        expect(viewer.message[:message]).to eq 'Access is restricted to the Special collections reading room. See Access conditions for more information.'
        expect(viewer.message[:type]).to eq 'location-restricted'
      end
    end
    ...
    context 'with location restiction on the file' do
      let(:contents) do
        [
          instance_double(Embed::Purl::Resource, three_dimensional?: true,
                                                 files: [instance_double(Embed::Purl::ResourceFile, download: 'location-based', title: 'obj1.glb', file_url: '//file/druid:abc123/obj1.glb', not_no_download?: false)])
        ]
      end

      it 'has no message' do
        expect(viewer.message).to be false
      end
    end
  end
end
