  ...
  describe '#media_wrapper' do
    describe 'data-stanford-only attribute' do
      it 'true for Stanford only files' do
        resource_file = double(Embed::PURL::Resource::ResourceFile, stanford_only?: true, location_restricted?: false, label: 'ignored')
      ...
      it 'false for public files' do
        resource_file = double(Embed::PURL::Resource::ResourceFile, stanford_only?: false, location_restricted?: false, label: 'ignored')
        media_wrapper = Capybara.string(subject_klass.send(:media_wrapper, file: resource_file))
    ...
    describe 'location restriction message' do
      it 'displayed when not in location' do
        resource_file = double(Embed::PURL::Resource::ResourceFile, stanford_only?: false, location_restricted?: true, label: 'ignored')
        media_wrapper = Capybara.string(subject_klass.send(:media_wrapper, file: resource_file))
    ...
    describe 'duration' do
      it 'sets the duration when the resource file has duration' do
        resource_file = double(Embed::PURL::Resource::ResourceFile, video_duration: Embed::PURL::Duration.new('PT0H1M2S'), label: 'ignored')
        media_wrapper = Capybara.string(subject_klass.send(:media_wrapper, file: resource_file))
      ...
      it 'leaves the duration empty when the resource file is missing duration' do
        resource_file = double(Embed::PURL::Resource::ResourceFile, label: 'ignored')
        media_wrapper = Capybara.string(subject_klass.send(:media_wrapper, file: resource_file))
    ...
    describe 'data-location-restricted attribute' do
      context 'stanford_only' do
        it 'true for location restricted files' do
          resource_file = double(Embed::PURL::Resource::ResourceFile, stanford_only?: true, location_restricted?: true, label: 'ignored')
          media_wrapper = Capybara.string(subject_klass.send(:media_wrapper, file: resource_file))
