  ...
  describe 'Purl::Resource' do
    ...
    describe 'files' do
      it 'returns an array of Purl::Resource::ResourceFile objects' do
        stub_purl_response_with_fixture(file_purl)
        expect(described_class.new('12345').contents.first.files.all?(Embed::Purl::Resource::ResourceFile)).to be true
      end
    end

    describe '#manifest_json_url' do
      it 'constructs a URL to a IIIF manifest' do
        expect(described_class.new('12345').manifest_json_url).to eq 'https://purl.stanford.edu/12345/iiif/manifest'
      end
    end
    ...
    describe '#manifest_json_response' do
      it 'fetches the IIIF manifest' do
        ...
        expect(described_class.new('12345').manifest_json_response).to eq '{}'
      end
      ...
      it 'raises an application error on failure' do
        ...
        expect { described_class.new('12345').manifest_json_response }.to raise_error(Embed::Purl::ResourceNotAvailable)
      end
    ...
    describe 'Purl::Resource::ResourceFile' do
      ...
      describe 'image?' do
        it 'returns true if the mimetype of the file is an image' do
          stub_purl_response_with_fixture(image_purl)
          expect(described_class.new('12345').contents.first.files.first).to be_image
        end

        it 'returns false if the mimetype of the file is not an image' do
          stub_purl_response_with_fixture(file_purl)
          expect(described_class.new('12345').contents.first.files.first).not_to be_image
        end
      end
      ...
      describe 'rights' do
        ...
        describe 'location_restricted?' do
          it 'identifies location restricted objects' do
            ...
            expect(described_class.new('12345').contents.first.files.all?(&:location_restricted?)).to be true
          end

          it 'identifies world accessible objects as not stanford only' do
            stub_purl_response_with_fixture(file_purl)
            expect(described_class.new('12345').contents.first.files.all?(&:location_restricted?)).to be false
          end
        ...
        describe 'world_downloadable?' do
          it 'is false for stanford-only objects' do
            stub_purl_response_with_fixture(stanford_restricted_file_purl)
            expect(described_class.new('12345').contents.first.files.all?(&:world_downloadable?)).to be false
          end
          ...
          it 'is true for identify world accessible objects' do
            stub_purl_response_with_fixture(file_purl)
            expect(described_class.new('12345').contents.first.files.all?(&:world_downloadable?)).to be true
          end
        end
      end
      ...
      describe 'video_data' do
        context 'valid videoData' do
          ...
          let(:video) { described_class.new('12345').contents.first.files.first }
