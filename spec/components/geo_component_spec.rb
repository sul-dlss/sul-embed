...
RSpec.describe GeoComponent, type: :component do
  let(:request) do
    Embed::Request.new(url: 'http://purl.stanford.edu/abc123')
  end
  let(:viewer) { Embed::Viewer::Geo.new(request) }
  let(:purl) { build(:purl, :geo) }
  let(:json) { { earthworks: true } }

  before do
    allow(Embed::Purl).to receive(:find).and_return(purl)

    stub_request(:get, 'https://purl.stanford.edu/cz128vq0535.meta_json')
  ...
  it 'draws geo html body for public resources' do
    ...
    expect(page).to have_css("a[href='https://earthworks.stanford.edu/catalog/stanford-abc123'] span", text: 'EarthWorks', visible: :all)
  ...
  context 'when not released to earthworks' do
    let(:json) { { earthworks: false } }

    it 'does not show external link' do
      expect(page).to have_no_css("a[href='https://earthworks.stanford.edu/catalog/stanford-abc123'] span", text: 'EarthWorks', visible: :all)
    end
  end
end
