...
describe 'embed this panel', js: true do
  ...
  describe 'embed code' do
    ...
    it 'includes the allowfullscreen no-scrolling, no-border, and no margin/padding attributes' do
      ...
      expect(page.find('.sul-embed-embed-this-panel textarea').value).to match(%r{<iframe.*frameborder='0'.*/>})
      expect(page.find('.sul-embed-embed-this-panel textarea').value).to match(%r{<iframe.*marginwidth='0'.*/>})
      expect(page.find('.sul-embed-embed-this-panel textarea').value).to match(%r{<iframe.*marginheight='0'.*/>})
      expect(page.find('.sul-embed-embed-this-panel textarea').value).to match(%r{<iframe.*scrolling='no'.*/>})
    ...
    it 'includes height and width attributes' do
      page.find('[data-sul-embed-toggle="sul-embed-embed-this-panel"]', match: :first).click
      expect(page.find('.sul-embed-embed-this-panel textarea').value).to match(%r{<iframe.*height='200px'.*/>})
      expect(page.find('.sul-embed-embed-this-panel textarea').value).to match(%r{<iframe.*width='100%'.*/>})
    end
  end
