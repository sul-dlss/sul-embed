  ...
  class Viewer
    class CommonViewer
      ...
      def header_html
        ...
        <<-HTML.strip_heredoc
          <div class="sul-embed-header">#{render_header_tools(String.new)}</div>
        HTML
      end
      ...
      def open_download_panel
        ...
        <<-HTML.strip_heredoc
          <button class="sul-embed-footer-tool sul-embed-btn sul-embed-btn-toolbar sul-embed-btn-default sul-i-download-3" aria-expanded="false" aria-label="open download panel" data-sul-embed-toggle="sul-embed-download-panel">
          </button>
          #{download_counter}
        HTML
      end
      ...
      def render_header_tools(html_string)
        header_tools_logic.each do |logic_method|
          if (tool = send(logic_method))
            html_string << send(tool, html_string)
          end
        end
        html_string
      end
      ...
      def header_title_html(html_string)
        <<-HTML.strip_heredoc
      ...
      def file_count_html(html_string)
