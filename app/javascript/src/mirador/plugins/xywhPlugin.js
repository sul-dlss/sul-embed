...
const onViewportChange = event => {
  ...
  if (itemCount > 1) {
    ...
    const [page1, page2] =
      item1.getBounds().x <= item2.getBounds().x
        ? [item1, item2]
        : [item2, item1]
    ...
    if (
      page1.source.width - page1_bounds.x < page1_bounds.width ||
      page2_bounds.x > 0
    ) {
      page2_bounds.width =
        page2_bounds.width - (page1.source.width - page1_bounds.x)
      visible_pages.push(convertToRegionUrl(page2_bounds, page2.source._id))
    }
    full_image = visible_pages.join("*")
  } else {
    try {
      const imageBounds = viewport.viewportToImageRectangle(
        viewport.getBounds(),
      )
